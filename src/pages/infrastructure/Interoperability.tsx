...
const standards = [
  {
    icon: FileJson,
    title: "HL7 FHIR R4",
    desc: "Native FHIR R4 support for modern healthcare data exchange. RESTful APIs, resource bundles, and subscription-based updates.",
    features: ["Patient resources", "Observation mapping", "Medication records", "Clinical documents"],
  },
  {
    icon: Layers,
    title: "HL7 v2.x",
    desc: "Legacy HL7 v2 message parsing and transformation. Seamless bridging between older systems and modern FHIR endpoints.",
    features: ["ADT messages", "ORU/ORM support", "Custom segment mapping", "Batch processing"],
  },
  {
    icon: RefreshCw,
    title: "CDA/C-CDA",
    desc: "Clinical Document Architecture support for continuity of care documents. Import, export, and transformation capabilities.",
    features: ["CCD documents", "Discharge summaries", "Referral notes", "Lab reports"],
  },
  {
    icon: Plug,
    title: "Custom APIs",
    desc: "Vendor-specific API adapters for major Canadian EMR systems. Oscar, Accuro, Telus Health, and more.",
    features: ["Oscar EMR", "Accuro", "Med Access", "PS Suite"],
  },
];
        ...
        <div className="grid grid-cols-1 md:grid-cols-[0.55fr_1.45fr] items-center split-layout-gap">
          ...
          >
            ...
            <p style={{ color: "rgba(255,255,255,0.72)", fontWeight: 400, fontSize: "clamp(15px, 1.25vw, 18px)", lineHeight: 1.55, maxWidth: "46ch" }}>
              Native HL7 FHIR, v2.x, and CDA support. Pre-built connectors for major Canadian EMRs. Data flows where it needs to go — securely and in real-time.
            </p>
          </motion.div>
          ...
          {standards.map((std, i) => (
            ...
            >
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: 18, color: "#111", letterSpacing: "-0.01em" }}>
                    {std.title}
                  </div>
                  <div style={{ fontWeight: 400, fontSize: 13, color: "rgba(30,30,30,0.65)", marginTop: 4, lineHeight: 1.5 }}>
                    {std.desc}
                  </div>
                </div>
              </div>
        ...
        <div className="flex flex-col md:flex-row items-stretch gap-4 justify-center">
          {integrationFlow.map((flow, i) => (
            <motion.div
              ...
              className="flex-1 rounded-[16px] p-5 relative"
              style={{
            ...
            >
              <div style={{ fontFamily: "monospace", fontSize: 11, color: "rgba(232,150,124,0.8)", fontWeight: 700, marginBottom: 8 }}>
                {flow.step}
              </div>
              <div style={{ fontWeight: 700, fontSize: 16, color: "rgba(255,255,255,0.95)", marginBottom: 4 }}>
                {flow.title}
              </div>
              ...
              {i < integrationFlow.length - 1 && (
                <ArrowRight className="hidden md:block absolute -right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-white/30" />
              )}
            </motion.div>
          ))}
        </div>
